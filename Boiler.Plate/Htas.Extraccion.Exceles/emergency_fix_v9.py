...
# Extrayendo la Azotea real V9
azotea_area = 200 # En base al Excel {12: 200}
terrazas_area = techado_total - azotea_area
azotea_html = '<div style="display: flex; border-bottom: 1px solid #333; width: 100%;">'
azotea_html += make_div(azotea_area / lote_total * 100, "#0f2c59", f"Amenidades {azotea_area}m²")
azotea_html += make_div(terrazas_area / lote_total * 100, "#d0e8f2", f"Terrazas Libres {int(terrazas_area)}m²")
...
for i in range(4, 45):
    ...
    for s_name, col in scenarios.items():
        ...
        elif 'Terreno.S.IGV' in val: fin_data[s_name]['costo_terreno'] = dfS.iloc[i, col]
        elif 'Costo_Construccion_S.IGV' in val: fin_data[s_name]['costo_construccion'] = dfS.iloc[i, col]
        elif 'Otros_Cost_Directos_S.IGV' in val: fin_data[s_name]['otros_costos'] = dfS.iloc[i, col]
...
def build_table(s_name, data, color):
    ingresos = data.get('ingresos', 1)
    cost_terr = data.get('costo_terreno', 0)
    cost_const = data.get('costo_construccion', 0)
    cost_otros = data.get('otros_costos', 0)
    ub = data.get('ub', 0)
    gop = data.get('gadmin', 0) + data.get('gventas', 0)
    ebt = data.get('ebt', 0)
    eat = data.get('eat', 0)
    tax = ebt - eat
    
    return f"""<h2 style="color: #0f2c59; margin-bottom: 5px;">P&L - Escenario {s_name}</h2>
<table style="width: 100%; border-collapse: collapse; font-size: 11pt; margin-top: 15px; margin-bottom: 15px;">
    <thead>
    ...
    <tbody>
        ...
        <tr>
            <td style="padding: 4px; border: 1px solid #ccc; padding-left: 20px;">- Costo Terreno</td>
            <td style="padding: 4px; border: 1px solid #ccc; text-align: right; color: #922b21;">({fc(cost_terr)})</td>
            <td style="padding: 4px; border: 1px solid #ccc; text-align: center; color: #922b21;">-{fp(cost_terr/ingresos)}</td>
        </tr>
        <tr>
            <td style="padding: 4px; border: 1px solid #ccc; padding-left: 20px;">- Costo Construcción</td>
            <td style="padding: 4px; border: 1px solid #ccc; text-align: right; color: #922b21;">({fc(cost_const)})</td>
            <td style="padding: 4px; border: 1px solid #ccc; text-align: center; color: #922b21;">-{fp(cost_const/ingresos)}</td>
        </tr>
        <tr>
            <td style="padding: 4px; border: 1px solid #ccc; padding-left: 20px;">- Otros Costos Directos</td>
            <td style="padding: 4px; border: 1px solid #ccc; text-align: right; color: #922b21;">({fc(cost_otros)})</td>
            <td style="padding: 4px; border: 1px solid #ccc; text-align: center; color: #922b21;">-{fp(cost_otros/ingresos)}</td>
        </tr>
...
all_tables = f"{t_base}\n\n<br>\n\n{t_opt}\n\n<br>\n\n{t_pes}"
pattern_fin = r'<h2[^>]*>P&L - Escenario BASE</h2>.*?<h2[^>]*>P&L - Escenario PESIMISTA</h2>.*?</table>'
content = re.sub(pattern_fin, all_tables, content, flags=re.DOTALL)

with open(md_path, "w", encoding="utf-8") as f:
    f.write(content)

print(f"Emergency Fix V9 applied.")
