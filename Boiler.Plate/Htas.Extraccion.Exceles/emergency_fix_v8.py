...
def get_typology_and_color(size):
    # Ya no redondeamos al asignar el texto, solo para la lógica de colores
...
azotea_html += make_div((lote_total - azotea_area) / lote_total * 100, "#d0e8f2", f"Terrazas Libres {int(lote_total - azotea_area)}m²")
azotea_html += '</div>\n'
...
for i in range(4, 38):
    ...
    for s_name, col in scenarios.items():
        ...
        elif 'Terreno.S.IGV' in val: fin_data[s_name]['cogs'] = fin_data[s_name].get('cogs', 0) + dfS.iloc[i, col]
        elif 'Costo_Construccion_S.IGV' in val: fin_data[s_name]['cogs'] = fin_data[s_name].get('cogs', 0) + dfS.iloc[i, col]
        elif 'Otros_Cost_Directos_S.IGV' in val: fin_data[s_name]['cogs'] = fin_data[s_name].get('cogs', 0) + dfS.iloc[i, col]
...
# REEMPLAZO. Sin page-breaks abusivos que dejen vaco.
all_tables = f"{t_base}\n\n<br>\n\n{t_opt}\n\n<br>\n\n{t_pes}"
...
print(f"Emergency Fix V8 applied.")
