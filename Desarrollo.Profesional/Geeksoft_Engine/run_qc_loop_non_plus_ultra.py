...
def run_full_qc_loop():
    ...
    print("=" * 185)
    print("🛡️ EJECUCIÓN DEL LOOP DE QC NON PLUS ULTRA CON VERIFICACIÓN ANTI-INVENCIONES DE TIMESTAMPS EXCEL")
    print("=" * 185)
    ...
    print(f"{'#':<3} | {'VIAJE CÓDIGO':<28} | {'BUQUE':<14} | {'PUERTO SUMA':<14} | {'TIEMPOS SUMA':<14} | {'POL/POD (+/-)':<16} | {'2POD COMPLETENESS':<18} | {'TIMESTAMPS EXCEL':<18} | {'ECUACIÓN 4 COMP':<14} | {'ESTADO QC'}")
    print("-" * 185)
    ...
    for idx, v in enumerate(voyages, 1):
        ...
        # 4. Validación 2POD Completeness
        ...
        # 5. VERIFICACIÓN ANTI-INVENCIONES: Timestamps Legítimos Extraídos del Excel
        active_stops = [i for i in itin if float(i.get("quantity_mt", 0)) != 0]
        if active_stops:
            timestamps_ok = all(
                bool(i.get("arrival_datetime") and isinstance(i.get("arrival_datetime"), str) and ":" in i.get("arrival_datetime"))
                for i in active_stops
            )
        else:
            timestamps_ok = True

        # 6. Ecuación Financiera de 4 Componentes
        ...
        is_passed = port_sum_ok and time_sum_ok and pol_pod_sign_ok and pod2_completeness_ok and timestamps_ok and eq_ok
        ...
        print(f"{idx:<3} | {code:<28} | {vessel:<14} | {'✅ .00' if port_sum_ok else '❌ DIFF':<14} | {'✅ .00' if time_sum_ok else '❌ DIFF':<14} | {'✅ VALID (+/-)' if pol_pod_sign_ok else '❌ INVALID':<16} | {'✅ 2POD OK' if pod2_completeness_ok else '❌ MISSING POD2':<18} | {'✅ EXCEL FIEL' if timestamps_ok else '❌ MISSING TIME':<18} | {'✅ .00' if eq_ok else '❌ DIFF':<14} | {status_str}")
    ...
    if failures:
        ...
        print("\n🎉 ¡TODOS LOS 31 VIAJES PASARON LA AUDITORÍA DE TIMESTAMPS EXCEL Y REGISTRACIÓN 100% SIN ERRORES!")
