"""
====================================================================================================
 📄 SCRIPT AUTÓNOMO — GENERADOR DE PDF OFICIAL DE AUDITORÍA: TERMINAL INTERACID MEJILLONES 🇨🇱
====================================================================================================
 Ubicación: Desarrollo.Profesional/Geeksoft_Engine/generate_pdf_mejillones_interacid_qc.py
 Propietario: Naviera Petral / Geeksoft Engine
 Estado: DINÁMICO 100% — MUELLAJE POR HORA $702.00/H PASSTHROUGH + LAUNCH CLEARANCE $0.00
====================================================================================================
"""
...
def get_dynamic_interacid_data():
    ...
    cur.execute("""
        ...
        WHERE port_id = 'MEJILLONES' AND vessel_id = 'MOQUEGUA' AND operation_type = 'DESCARGA' 
          AND sub_operation_type = 'MAIN' AND terminal_id = 'INTERACID'
        LIMIT 1;
    """)
...
def build_mejillones_interacid_pdf_html():
    base_dir = os.getcwd()
    ...
    d = get_dynamic_interacid_data()
    
    dockage_interacid = round(702.00 * d["total_hours"], 2) # $25,272.00
    light_dues_chile = round(4.07 * d["grt"] / 15.0, 2)
    pilotage_cost = round(0.15 * d["grt"], 2)
    
    shifting_total = pilotage_cost + 11200.00 + 1742.50
    general_total = light_dues_chile + dockage_interacid + 390.00 + 272.57 + 1800.00 + 0.00 + 280.00 + 330.00 + 650.00 + 700.00 + 1140.35 + 29.00 + 110.00 + 3264.40
    agency_total = 1200.00
    
    total_auditado = shifting_total + general_total + agency_total # $52,048.70 USD
...
<head>
    <meta charset="UTF-8">
    <title>Acta Auditoría Terminal Interacid Mejillones 🇨🇱</title>
    <style>
...
<body>
    ...
    <table class="header-table">
        <tr>
            ...
            <td style="width: 50%;" class="title-header">
                ...
                <span style="font-size: 8pt; font-weight: bold; color: #000000;">ACTA DE AUDITORÍA OFICIAL — TERMINAL INTERACID MEJILLONES 🇨🇱</span>
            </td>
    ...
    <div class="box-container">
        <div class="box-title">📋 [INPUTS DE NAVEGACIÓN & AUDITORÍA P×Q — INTERACID MEJILLONES (DINÁMICO DB)]:</div>
  ...
  • TERMINAL / AGENCIA:     Terminal Interacid Mejillones 🇨🇱 | Agente: B&M Agencia | Remolques: Ultratug Ltd. (4 Maniobras)
  • OPERACIÓN & CARGA:      Descarga {d['cargo_tons']:,.0f} MT | Horas Puerto: {d['total_hours']:.1f} Horas en Muelle
  • REGLA MUELLAJE HORARIO: $702.00 USD / hora x {d['total_hours']:.1f}h Estancia = ${dockage_interacid:,.2f} USD
        </pre>
    </div>

    <div style="font-weight: bold; font-size: 7.8pt; margin-top: 4px; margin-bottom: 2px;">
        📊 [DESGLOSE DE LIQUIDACIÓN AUDITADA — TERMINAL INTERACID]:
    </div>
    ...
    <table class="audit-table">
        <thead>
            <tr>
                <th style="width: 4%;">#</th>
                <th style="width: 32%;">CONCEPTO OFICIAL INTERACID</th>
        ...
        <tbody>
            ...
            <tr><td>3</td><td class="font-bold">Linesmen (Amarre y Desamarre)</td><td>Amarradores Interacid</td><td>$871.25 / mnvr</td><td>$871.25 x 2 eventos</td><td class="text-right font-bold">$1,742.50</td></tr>
            ...
            <tr class="passthrough-row"><td>5</td><td>Dockage / Muellaje Interacid [*PASSTHROUGH 🟨]</td><td>Terminal Interacid</td><td>$702.00 / hora</td><td>$702.00 x {d['total_hours']:.1f}h Estancia</td><td class="text-right font-bold">${dockage_interacid:,.2f}</td></tr>
            ...
            <tr><td>9</td><td>Launch Clearances (Incluido)</td><td>B&M Agencia</td><td>$0.00 Flat</td><td>$0.00 Incluido en Paquete</td><td class="text-right font-bold">$0.00</td></tr>
            ...
            <tr><td>14</td><td>ISPS Fee Interacid</td><td>Terminal Interacid</td><td>$1,140.35 Flat</td><td>$1,140.35 Flat</td><td class="text-right font-bold">$1,140.35</td></tr>
            ...
            <tr><td>17</td><td>Loading Master Interacid</td><td>Terminal Interacid</td><td>$3,264.40 Flat</td><td>$3,264.40 Flat</td><td class="text-right font-bold">$3,264.40</td></tr>
            ...
            <tr class="total-row">
                <td colspan="5" class="text-right" style="padding: 6px;">TOTAL GENERAL AUDITADO TERMINAL INTERACID:</td>
  ...
  • LIQUIDACIÓN EXPERTA SANDRA (EXCEL):      $52,048.70 USD [ALOJADO CON 100% DE PRECISIÓN EN EL EXTREMO MÍNIMO]
  ...
  • ESCENARIO PESIMISTA (MÁXIMO - OVERTIME):  ${pesimista_total:,.2f} USD (Con Remolcador/Lancha Overtime Adicional)
    ...
    <div style="font-size: 6.8pt; font-weight: bold; background-color: #fef08a; border: 1px solid #eab308; padding: 4px 6px; margin-top: 4px;">
        * NOTA DE PASSTHROUGH: El ítem #5 (Muellaje Interacid por ${dockage_interacid:,.2f} USD) es 100% refacturable a cliente Southern Perú según acuerdo. Impacto neto en PnL de Petral = $0.00 USD.
    </div>
...
"""
    output_path = os.path.join(os.getcwd(), "Desarrollo.Profesional", "Obsidian.Maestro.Costos.Portuarios", "06_QC", "Mejillones_Interacid_Proforma_Auditoria_Official.html")
    with open(output_path, "w", encoding="utf-8") as f:
        f.write(html_content)
    print(f"[SUCCESS] Plantilla HTML/PDF oficial de Mejillones Interacid creada en:\n{output_path}")
    return output_path

if __name__ == "__main__":
    build_mejillones_interacid_pdf_html()
