...
|---|---|---|
| 📜 [run_qc_super_loop_react_grid_reconciliation.py](file:///C:/Users/rguti/PETRAL.SMART.DASHBOARD/Desarrollo.Profesional/Geeksoft_Engine/run_qc_super_loop_react_grid_reconciliation.py) | **Super Loop QC Reconciliación React (60/60 Rutas):** Valida el ciclo de vida completo Frontend-Backend (cruce de `projectionLines` en memoria vs `aggregated_data`), garantizando 0 ceros fantasmas, viajes > 0 y P&L visible en todas las rutas (bunkering, multi-drop y spot). | `python run_qc_super_loop_react_grid_reconciliation.py` |
