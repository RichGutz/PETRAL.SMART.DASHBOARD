...
def generate_multicotizador_flowchart():
    ...
    subgraph cluster_lvl1 {
        ...
        DB_RoutesClients [label="⚓ MAESTRO RUTAS CLIENTES (routes_clients)\\nOrigen, Destino, Distancias NM\\nRutas Oficiales Clientes Activos", shape=cylinder, fillcolor="#B2EBF2", penwidth=2];
        ...
        DB_RoutesQuotes [label="📁 MAESTRO COTIZACIONES PROSPECTOS (routes_quotes)\\nRutas simuladas, prospectos comercial\\ny versionado de cotizaciones", shape=cylinder, fillcolor="#C8E6C9", penwidth=2];
    ...
    subgraph cluster_lvl2 {
        label = "NIVEL 2: PROVEEDORES DE DATOS Y RESOLUCIÓN BÚNKER (Frontend Services)"; 
        ...
        S_Port [label="⚙️ portCostsRatesService\\nResuelve tarifa fija desde port_cost_static", shape=component, fillcolor="#B3E5FC", penwidth=2];
        S_Storage [label="💾 multicotizadorStorageService\\nGraba en routes_clients (Activos)\\no en routes_quotes (Prospectos)", shape=component, fillcolor="#A5D6A7", penwidth=2];
        S_Retriever [label="🔍 multicotizadorRetrieverService\\nCarga cotizaciones desde routes_quotes", shape=component, fillcolor="#81C784", penwidth=2];
    ...
    subgraph cluster_lvl3 {
        label = "NIVEL 3: CABECERA COMERCIAL Y GRILLA SPREADSHEET (MultiCotizadorExcel.tsx)"; 
        style="filled"; color="#0284C7"; fontcolor="#F3E5F5";
        
        { rank=same; UI_Step1; UI_Step2; UI_Step3; UI_FactSheet; UI_Grid; UI_MuellajeCell; }
        ...
        UI_MuellajeCell [label="⚖️ Celda Dual de Muellaje\\nSub-celda 1: Monto $ USD | Sub-celda 2: Checkbox [x]", shape=box, fillcolor="#BBDEFB", penwidth=2];

        UI_Step1 -> UI_Step2 -> UI_Step3 -> UI_FactSheet -> UI_Grid -> UI_MuellajeCell [style=invis];
    }
    ...
    DB_RoutesClients -> S_Route [label=" Rutas Clientes Activos"];
    ...
    DB_RoutesQuotes -> S_Retriever [label=" Cotizaciones Prospectos"];
    ...
    API_SaveEndpoint -> DB_RoutesQuotes [label=" Save Quote", color="#2E7D32"];
    API_SaveEndpoint -> DB_RoutesClients [label=" Save Route", color="#2E7D32"];
