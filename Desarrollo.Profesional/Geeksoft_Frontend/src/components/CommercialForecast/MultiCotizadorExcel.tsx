...
export const MultiCotizadorExcel: React.FC<{ portCostMode?: 'static' | 'matrix' }> = ({ portCostMode = 'static' }) => {
    ...
    // Helper dinámico para agrupar y catalogar puertos activos por OP.DEST (CARGAR / DESCARGAR)
    const getDynamicPortCostItems = () => {
        ...
        puertosConfig.forEach((p, idx) => {
            const portId = idx === 0 
                ? (tramos[0]?.origin_port_id || '') 
                : (tramos[idx - 1]?.destination_port_id || '');
            ...
            if (result?.tramos && result.tramos.length > 0) {
                if (idx === 0) {
                    cost = result.tramos[0]?.agency_costs_origin || 0;
                } else {
                    cost = result.tramos[idx - 1]?.agency_costs_destination || 0;
                }
            } else {
                cost = p.manual_port_cost ? Number(p.manual_port_cost) : 0;
            }

            const action = p.action;
            let roleTag = 'PORT';
            let role: 'POL' | 'POD' | 'PORT' = 'PORT';
            ...
            items.push({
                port_id: portId,
                action,
                label: `${roleTag} (${portId})`,
                cost,
                role
            });
        });

        if (items.length === 0 && tramos.length > 0) {
            const oPort = tramos[0]?.origin_port_id || 'POL';
            const dPort = tramos[0]?.destination_port_id || 'POD';
            const cPOL = result?.tramos?.[0]?.agency_costs_origin || 0;
            const cPOD = result?.tramos?.[0]?.agency_costs_destination || 0;
            items.push({ port_id: oPort, action: 'CARGAR', label: `POL (${oPort})`, cost: cPOL, role: 'POL' });
            items.push({ port_id: dPort, action: 'DESCARGAR', label: `POD (${dPort})`, cost: cPOD, role: 'POD' });
        }
            ...
            <div className="overflow-x-auto border border-slate-300 rounded bg-white shadow-sm flex flex-col mb-3">
                <table className="w-full border-collapse text-[12px] font-mono table-fixed select-text">
                    ...
                    <tbody>
                        ...
                        <tr className="bg-blue-100/80 border-t-2 border-blue-400 h-8 select-none font-black text-blue-950 text-xs">
                            <td colSpan={3} className="border-r border-blue-200 text-left pl-3 font-sans text-[10.5px] uppercase tracking-wide text-blue-950 bg-blue-200/50">📊 TOTAL ESTIMADO (MOTOR)</td>
                            <td className="border-r border-blue-200 text-right pr-2 font-mono">
                        ...
                        {(() => {
                            ...
                            return (
                                <>
                                    ...
                                    {/* Fila 3 de Totales: Diferencia / Discrepancia (Δ) */}
                                ...
                                {(() => {
                                    ...
                                    return (
                                        <>
                                            ...
                                            {/* 1.b Refacturación de Muellaje (Formato sutil secundario debajo de Flete Bruto) */}
