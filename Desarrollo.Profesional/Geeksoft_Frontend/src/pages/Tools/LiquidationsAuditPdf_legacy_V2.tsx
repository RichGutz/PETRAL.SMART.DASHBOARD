...
import { LiquidationsExecutivePdfAudit_legacy } from '../../components/CommercialForecast/LiquidationsExecutivePdfAudit_legacy';
import { ForecastService } from '../../services/api';
import { FileCheck2, Loader2, Database } from 'lucide-react';

export const LiquidationsAuditPdf_legacy_V2: React.FC = () => {
    ...
    useEffect(() => {
        const fetchLiquidations = async () => {
            try {
                ...
                console.error("Error al cargar liquidaciones reales para reporte PDF legacy:", err);
        ...
        <section className="flex flex-col flex-1 gap-4 relative mt-2 animate-in fade-in slide-in-from-bottom-2 duration-300 w-full max-w-full">
            ...
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-base font-black text-slate-900 tracking-tight uppercase">AUDITORÍA PDF DE LIQUIDACIONES REALES (LEGACY)</h2>
                        <p className="text-[11px] text-slate-500 font-medium">Generación de Acta Oficial PDF A4 Landscape comparando los 31 viajes reales vs simulación Spot Matrix Mode (Versión Legacy)</p>
                    </div>
                </div>
            ...
            {loading ? (
                <div className="flex flex-col items-center justify-center h-80 bg-white rounded-xl border border-slate-200 space-y-3">
                    ...
                    <p className="text-xs font-bold text-slate-600">Cargando 31 viajes de ejecuciones reales para el reporte PDF legacy...</p>
                </div>
            ) : error ? (
                ...
                <LiquidationsExecutivePdfAudit_legacy liquidations={liquidations} />
            )}
