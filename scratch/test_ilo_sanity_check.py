...
from backend.port_engines import calculator_ilo

print("=" * 80)
print(" *** SANITY CHECK EMPIRICO: PUERTO DE ILO (SPCC / ENAPU / PSA MARINE / PETRANSO) ***")
print("=" * 80)
...
fleet = [
    ...
    },
    {
        ...
        "grt": 11773,
        "port_hours": 37.0,
        "expected_total": 24490.30
    }
]

for vessel in fleet:
    vname = vessel["vessel_name"]
    res = calculator_ilo.run(vessel, port_hours=vessel["port_hours"])
    ...
    assert diff < 2.0, f"ERROR en Ilo para {vname}: Obtenido ${calculated:,.2f} vs Esperado ${expected:,.2f}"
    print(f" [OK] {vname} Coincide 100% centavo a centavo")

print("\n" + "=" * 80)
print(" [EXITO TOTAL SANITY CHECK] MOTOR DE ILO COINCIDE 100% CON LA IMAGEN DE LA EXPERTA")
print("=" * 80)
