...
# 1. Update getDynamicPortCostItems to ignore cost < 1 (including 0.00001 dummy epsilon)
old_get_items = """            if (action === 'CARGAR') {
                role = 'POL';
                roleTag = 'POL';
...
new_get_items = """            if (action === 'CARGAR') {
                role = 'POL';
                roleTag = 'POL';
            } else if (action === 'DESCARGAR') {
                role = 'POD';
...
# 2. Update Card 3 threshold check to cost >= 1
...
# 3. Update Card 4 threshold check to cost >= 1
...
print("EPSILON DUMMY ZEROES ELIMINATED SUCCESSFULLY!")
