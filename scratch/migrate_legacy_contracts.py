...
API_URL = "https://forecast.geeksoft.tech/api/v1/forecast/spot/save"

contracts_to_migrate = [
    # 1. NEXA CALLAO-MEJILLONES (2025)
    {
        "name": "NEXA.CALLAO.MEJILLONES.2025.V1",
    ...
    {
        "name": "SPCC.ILO.MATARANI.2025.V1",
    ...
    {
        "name": "SPCC.ILO.MEJILLONES.2025.V1",
        "description": "Contrato Registrado (contracts) - Cliente SPCC",
        "pais": "PE",
        "is_prospect": False,
        "is_contract": True,
    ...
    {
        "name": "NEXA.CALLAO.MATARANI.2027.V1",
        "description": "Contrato Registrado (contracts) - Cliente NEXA",
        "pais": "PE",
        "is_prospect": False,
        "is_contract": True,
...
def run_migration():
    print(f"Iniciando migracion de {len(contracts_to_migrate)} contratos legacy (NEXA & SPCC) al nuevo formato Multicotizador...")
    success_count = 0
    
    for c in contracts_to_migrate:
        name = c["name"]
        try:
            resp = requests.post(API_URL, json=c, timeout=10)
            if resp.status_code == 200:
                print(f"  [OK] Migrado con exito: {name}")
                success_count += 1
            else:
                print(f"  [ERROR] HTTP {resp.status_code} migrando {name}: {resp.text}")
        except Exception as e:
            print(f"  [EXCEPCION] Migrando {name}: {e}")

    print(f"\nMigracion finalizada: {success_count}/{len(contracts_to_migrate)} contratos enriquecidos creados en Supabase DB.")
