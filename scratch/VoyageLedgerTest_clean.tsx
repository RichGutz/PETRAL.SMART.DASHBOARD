...
const renderBadges = (dbString: string) => {
    return dbString.split('┬À').map(s => s.trim()).map((table, idx) => {
...
export const VoyageLedgerTest: React.FC<{ portCostMode?: 'static' | 'matrix' }> = ({ portCostMode = 'static' }) => {
    ...
    if (loading) return <div className="p-8 text-center text-slate-500 font-semibold animate-pulse">Iniciando Motor de Auditor├¡a...</div>;
    ...
    // L├¡mites reales del tarifario (brackets de contrato SPCC_2025)
    const minIntake = 10000;
    const maxIntake = 14500;
    // Por defecto el input muestra el valor del viaje, topado al m├íximo del tarifario
    ...
    const renderScenarioContent = (
        ...
        const formatCurrency = (val: any) => {
            ...
            return isNaN(num) ? 'ÔÇö' : new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(num);
        };
        const formatNumber = (val: any) => {
            const num = parseFloat(val);
            return isNaN(num) ? 'ÔÇö' : new Intl.NumberFormat('en-US', { maximumFractionDigits: 4 }).format(num);
        };

        const auditRows = [
            { metric: "1. Ritmo Carga (MT/hr)",  key: "1. Ritmo Carga (act_load)",       gk: scenarioResult.actual_load_rate,      ptr: scenarioPetral.act_load,  isCurr: false, db: "contracts ┬À vessels ┬À ports", ui: "Contratos / Flota / Puertos" },
            { metric: "2. Ritmo Desc. (MT/hr)", key: "2. Ritmo Descarga (act_disch)",  gk: scenarioResult.actual_discharge_rate, ptr: scenarioPetral.act_disch, isCurr: false, db: "contracts ┬À vessels ┬À ports", ui: "Contratos / Flota / Puertos" },
            { metric: "3. D├¡as de Puerto",       key: "3. D├¡as de Puerto (port_days)",  gk: scenarioResult.port_days_unit,          ptr: scenarioPetral.port_days, isCurr: false, db: "ports ┬À Calculado",  ui: "Motor" },
            { metric: "4. D├¡as de Mar",          key: "4. D├¡as de Mar (sea_days)",      gk: scenarioResult.sea_days_unit,           ptr: scenarioPetral.sea_days,  isCurr: false, db: "routes ┬À vessels",              ui: "Maestro Rutas / Flota" },
            { metric: "5. D├¡as de Viaje",        key: "5. D├¡as de Viaje (tot_dur)",     gk: scenarioResult.total_duration_unit,     ptr: scenarioPetral.total_duration,   isCurr: false, db: "Calculado",                     ui: "Motor" },
            { metric: "6. Income",               key: "6. Income (income)",              gk: scenarioResult.net_income,              ptr: scenarioPetral.net_income,isCurr: true,  db: "contracts ┬À contract_tariffs",     ui: "Contratos / Tarifario" },
            ...
            { metric: "8. Costo Bunker",          key: "8. Costo Bunker (bunker)",        gk: scenarioResult.total_bunker_costs_unit, ptr: scenarioPetral.bunker_costs,    isCurr: true,  db: "vessels ┬À bunker_prices",           ui: "Maestro Flota / Bunker" },
            ...
            { metric: "10. Voyage Result",        key: "10. Voyage Result (voy_res)",     gk: scenarioResult.voyage_result,           ptr: scenarioPetral.voyage_result,   isCurr: true,  db: portCostMode === 'static' ? "contract_tariffs ┬À port_cost_static" : "contract_tariffs ┬À port_costs_matrix", ui: "Tarifas / Costos Portuarios" },
                ...
                {isPrint && (
                    <div className="border-b border-slate-800 pb-1 mb-1 flex justify-between items-center">
                        <h2 className="text-xs font-black text-slate-800 uppercase tracking-wide">
                            ­ƒº¬ GEEKSOFT Voyage Ledger Auditor├¡a: <span className="text-blue-600">{vesselName.replace('_', ' ')}</span> &rarr; <span className="text-purple-600">{originPort} - {destPort}</span>
                        </h2>
                    ...
                    <div className="flex-1 flex flex-col gap-1">
                        <div className={`border rounded-lg shadow-sm overflow-hidden ${COLOR_SCHEME.bunker_prices.cardBg} ${COLOR_SCHEME.bunker_prices.border}`}>
                            ...
                            <div className="p-3 flex flex-col gap-1.5">
                                <div className="flex justify-between items-baseline"><span className={`font-semibold text-[10px] uppercase ${COLOR_SCHEME.bunker_prices.text}`}>Fecha Cotizaci├│n</span><span className="font-mono text-slate-800 font-bold text-xs">{scenarioResult.raw_inputs?.bunker_price_date || 'ÔÇö'}</span></div>
                        ...
                        <div className={`flex-1 flex flex-col border rounded-lg shadow-sm overflow-hidden ${COLOR_SCHEME.agency_matrix.cardBg} ${COLOR_SCHEME.agency_matrix.border}`}>
                            ...
                            <div className="p-3 flex flex-col gap-1.5 flex-1 justify-between">
                                ...
                                <div className="flex justify-between items-baseline border-t border-dashed border-rose-200 pt-1 mt-0.5">
                                    <span className="font-semibold text-[10px] uppercase text-rose-600">Ôå│ Loading Master</span>
                                        ...
                                        <div className="flex flex-col gap-1 justify-start">
                                            <div className="flex justify-between items-center">
                                                ...
                                                <div className="flex items-center gap-1">
                                                    ...
                                                    />
                                                    <span className={`text-[10px] font-bold ${simulating ? 'text-amber-500 animate-pulse' : 'text-slate-500'}`}>MT{simulating ? ' Ôƒ│' : ''}</span>
                                                </div>
                                            </div>
                    ...
                    <table className="w-full text-left text-sm border-collapse table-fixed">
                        <thead className="sticky top-0 z-10 shadow-sm">
                            <tr className="bg-slate-100 border-b border-slate-300 text-slate-700">
                                <th className="p-2 font-bold" style={{width:'13%'}}>M├®trica</th>
                                <th className="p-2 font-bold" style={{width:'25%'}}>F├│rmula Algor├¡tmica</th>
                                <th className="p-2 font-bold" style={{width:'20%'}}>Reemplazo Num├®rico</th>
                                ...
                                <th className="p-2 font-bold text-center" style={{width:'9%'}}>Delta (╬ö)</th>
                    ...
                    <div className="flex mt-6 mb-2 px-2 flex-col text-sm page-break-inside-avoid">
                        <div className="grid grid-cols-2 gap-12">
                            ...
                            <div className="flex flex-col">
                                <p className="font-bold text-slate-700 mb-1">Comentarios / Justificaci├│n:</p>
        ...
        <>
            ...
            <div className="no-print bg-white rounded-lg shadow-sm border border-slate-200 p-6">
                {renderScenarioContent(
                    ...
                    <button
                        onClick={() => {
                            if (!runResult || !runResult.audit_trail) { alert('No hay datos cargados a├║n.'); return; }
                            ...
                            const auditRowsPrint = [
                                ...
                                { metric: '3. D├¡as de Puerto',       key: '3. D├¡as de Puerto (port_days)', gk: runResult.port_days_unit,          isCurr: false },
                                { metric: '4. D├¡as de Mar',          key: '4. D├¡as de Mar (sea_days)',     gk: runResult.sea_days_unit,           isCurr: false },
                                { metric: '5. D├¡as de Viaje',        key: '5. D├¡as de Viaje (tot_dur)',     gk: runResult.total_duration_unit,     isCurr: false },
                            ...
                            <div class="cards-grid">
                                ...
                                <div class="card card-orange">
                                    <div class="card-header">L├¡mites Portuarios <span class="card-badge">ports</span></div>
                                ...
                                <div class="card card-amber">
                                    ...
                                    <div class="card-row"><span>Fecha Cotizaci├│n</span><strong>${ri.bunker_price_date||'N/A'}</strong></div>
                            ...
                            <div class="header-bar">
                                <div style="flex:1">
                                    <h1>GEEKSOFT Voyage Ledger ÔÇö Auditor├¡a Matem├ítica &nbsp;|&nbsp; Barco: ${v.replace('_',' ')} &nbsp;|&nbsp; Ruta: ${o} ÔåÆ ${d} &nbsp;|&nbsp; Per├¡odo: 2026-07 &nbsp;|&nbsp; Generado: ${fechaStr}</h1>
                                </div>
                                <span class="badge">PETRAL ┬À ACTA DE CONFORMIDAD</span>
                            </div>
                            ${cardsHTML}
                            <table>
                                <thead><tr>
                                    <th style="width:13%">M├®trica</th>
                                    <th style="width:27%">F├│rmula Algor├¡tmica</th>
                                    <th style="width:22%">Reemplazo Num├®rico</th>
                                    <th style="width:12%;text-align:center">GEEKSOFT (Motor)</th>
                                    <th style="width:13%;text-align:center">PETRAL (Excel)</th>
                                    <th style="width:13%;text-align:center">Delta (╬ö)</th>
                                </tr></thead>
                                <tbody>${tableRows}</tbody>
                            </table>
                            <div class="acta">
                                <div class="acta-title">Ô£ì´©Å Acta de Conformidad Matem├ítica ÔÇö Firmas y Validaci├│n</div>
                                <div class="acta-grid">
                                    ...
                                    <div style="display:flex;flex-direction:column;">
                                        <div class="field-label">Comentarios / Justificaci├│n de divergencias:</div>
                            ...
                            else { alert('El navegador bloque├│ la ventana emergente. Habil├¡tala para este sitio.'); }
                        }}
                    ...
                    >
                        <span className="text-xl">­ƒû¿´©Å</span>
                ...
                {SCENARIOS.map((sc, idx) => {
                    ...
                    return (
                        <div key={idx} className="page-break flex flex-col justify-between" style={{ height: '95vh', padding: '20px' }}>
                            ...
                            {/* Bloque de validaci├│n y firmas (Footer para impresi├│n) */}
                            <div className="mt-4 pt-4 border-t-2 border-slate-800 shrink-0">
                                ...
                                <div>
                                    <div className="font-bold text-slate-800 mb-2 text-sm">Comentarios (Raz├│n de la divergencia / Observaciones):</div>
