/**
 * LOOP QC BENOIT BLANC: AUDITORÍA CRUZADA MULTICOTIZADOR VS MATRIZ FINANCIERA
 * Prueba matemática de convergencia 1:1 de P&L ante variaciones de Demora (Días)
 */

async function runCrossCheck() {
    console.log('================================================================================');
    console.log('   [LOOP QC BENOIT BLANC] AUDITORÍA CRUZADA: MULTICOTIZADOR VS MATRIZ');
    console.log('   Comparación Matemática de P&L ante Inyección de Días de Demora');
    console.log('================================================================================\n');

    // 1. Descargar catálogo oficial de rutas y buques
    console.log('[1/3] Descargando catálogo de rutas en vivo desde la API...');
    ...
    const demurrageTestDays = [0.0, 0.5, 1.0, 1.5, 2.0, 3.0, 4.5, 5.0];
    let totalAssertions = 0;
    let successfulAssertions = 0;
    let testedRoutesCount = 0;

    console.log('\n[2/3] Ejecutando simulación comparativa de P&L para todas las rutas...\n');
    ...
    for (const route of spotRoutes) {
        ...
        for (const days of demurrageTestDays) {
            totalAssertions++;

            // A. MULTICOTIZADOR ENGINE
            const multiDemurrageRevenue = days * demRate;
            ...
            const multiDemurrageIfoTons = days * vParams.consumption_idle_ifo;
            const multiDemurrageMdoTons = days * vParams.consumption_idle_mdo;
            ...
            const multiTotalDays = baseDuration + days;
            ...
            // B. MATRIZ FINANCIERA (FORECAST GRID)
            const gridEffectiveDemurrageDays = days;
            const gridDemurrageIncome = days * demRate;
            const gridGrossRevenue = baseFreight + baseDockage + gridDemurrageIncome;
            ...
            // C. COMPARACIÓN Y ASERCIÓN PERICIAL
            const delta = Math.abs(multiVoyageResultPnl - gridPlVsRequired);

            if (delta < 0.01) {
                successfulAssertions++;
            } else {
                console.error('[ERROR DE CONVERGENCIA] Ruta: ' + (route.name || route.route_name) + ' | Días Demora: ' + days);
    ...
    console.log('[3/3] Resultados del Test de Convergencia Cruzada:');
    console.log('      - Rutas Oficiales Auditadas:      ' + testedRoutesCount);
    console.log('      - Casos de Días de Demora:        ' + demurrageTestDays.length + ' ([' + demurrageTestDays.join(', ') + '] días)');
    ...
    if (successRate === 100.00) {
        console.log('   [APROBADO] BLINDAJE MATEMÁTICO TOTAL: MULTICOTIZADOR Y MATRIZ SON 100% IDÉNTICOS');
    } else {
