...
print("--- INSPECCION DE PIERNAS EN AGGREGATED DATA ---")
    ...
    for route_key, vessels in routes.items():
        for vessel_name, months in vessels.items():
            for m, metrics in list(months.items())[:1]:
                route_name = metrics.get('route_name')
                route_id = metrics.get('raw_inputs', {}).get('route_id')
                full_name = route_name or route_id or route_key
                print(f"\nCliente: {client} | Barco: {vessel_name} | RouteKey: '{route_key}'")
