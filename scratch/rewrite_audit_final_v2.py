...
# The state section to replace starts at `export const VoyageLedgerUniversal...`
# And ends before `const COLOR_SCHEME = {`
split1 = content.split('export const VoyageLedgerUniversal')
part1 = split1[0]
part2 = split1[1]

split2 = part2.split('const COLOR_SCHEME = {')
part2_rest = 'const COLOR_SCHEME = {' + split2[1]
...
# Now we find where the MAIN component `return` block starts.
# Inside `part2_rest`, `renderScenarioContent` is defined, and then the main `return (` is at the bottom.
match = re.search(r'(const renderScenarioContent = \(.*?\);.*?)(?=\s*return \(\s*<>\s*<div className="flex justify-between items-center mb-6 print:hidden">)', part2_rest, re.DOTALL)
if not match:
    # Let's try a generic search for the last `return (` that is at the outer block level.
    # The return of the component is after `renderScenarioContent`. Let's just split by `    return (` which is 4 spaces indented.
    splits = part2_rest.split('\n    return (')
    if len(splits) >= 2:
