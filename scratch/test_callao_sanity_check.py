...
from backend.port_engines import calculator_callao

print("=" * 80)
print(" *** SANITY CHECK EMPIRICO: PUERTO DE CALLAO (APM TERMINALS) ***")
print("=" * 80)
...
fleet = [
...

    {
        ...
        "grt": 11773,
        "port_hours": 34.0,
        "expected_total": 17573.30
    }
]

for vessel in fleet:
    vname = vessel["vessel_name"]
    res = calculator_callao.run(vessel, port_hours=vessel["port_hours"], inputs={"is_foreign": False})
    ...
    assert diff < 2.0, f"ERROR en Callao para {vname}: Obtenido ${calculated:,.2f} vs Esperado ${expected:,.2f}"
    print(f" [OK] {vname} Coincide 100% centavo a centavo")

print("\n" + "=" * 80)
print(" [EXITO TOTAL SANITY CHECK] MOTOR DE CALLAO COINCIDE 100% CON LA IMAGEN DE LA EXPERTA")
print("=" * 80)
