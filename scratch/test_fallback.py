...
def test():
    ...
    scenario_res = res["aggregated_data"]["SPCC"]["ILO-MATARANI"]["CONCON_TRADER"]["2026-07"]
    
    print("=== TEST FALLBACK: CONCON_TRADER (DESDE AGENCY_MATRIX) ===")
    print(f"Costo Origen (ILO): ${scenario_res['raw_inputs'].get('agency_costs_origin'):,.2f} USD")
    print(f"Costo Destino (MATARANI): ${scenario_res['raw_inputs'].get('agency_costs_destination'):,.2f} USD")
    print(f"Desglose de Origen (ILO): {scenario_res.get('port_costs_breakdown', {}).get('origin')}")
    print(f"Desglose de Destino (MATARANI): {scenario_res.get('port_costs_breakdown', {}).get('destination')}")
