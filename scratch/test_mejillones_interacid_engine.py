...
from backend.port_engines import calculator_mejillones_interacid, core

print("=" * 80)
print(" *** VERIFICACION EMPIRICA EN TERMINAL: TERMINAL INTERACID (MEJILLONES - CHILE) ***")
print("=" * 80)
...
fleet = [
    ...
    },
    {
        ...
        "grt": 11773,
        "port_hours": 36.0,
        "expected_total": 71341.07
    }
]

for vessel in fleet:
    vname = vessel["vessel_name"]
    res = calculator_mejillones_interacid.run(vessel, port_hours=vessel["port_hours"])
    ...
    assert diff < 2.0, f"ERROR en Terminal Interacid para {vname}: Obtenido ${calculated:,.2f} vs Esperado ${expected:,.2f}"
    print(f" [OK] {vname} Coincide 100% centavo a centavo")

print("\n" + "=" * 80)
print(" [EXITO TOTAL] MOTOR DE TERMINAL INTERACID COINCIDE 100% CON LA EXPERTA")
print("=" * 80)
