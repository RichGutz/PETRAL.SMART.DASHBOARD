...
part1 = content.split('export const VoyageLedgerUniversal')[0]
...
part1 = part1.replace("import React, { useEffect, useState } from 'react';", "import React, { useEffect, useState } from 'react';\n" + import_inject)

match = re.search(r'(const renderScenarioContent = \(.*?\);.*?)(?=\s*return \()', content, re.DOTALL)
if match:
                ...
                if (tr) {
                    ...
                    if (cMatch && cMatch.tariffs) {
                        ...
                        if (tMatch) {
                            ...
                            newConf[idx].freight_rate = 0; // reset if out of range
                        }
                    }
                }
            }
...
with open('Desarrollo.Profesional/Geeksoft_Frontend/src/components/CommercialForecast/VoyageLedgerFinal.tsx', 'w', encoding='utf-8') as f:
    f.write(part1 + top_section + render_func + "\n" + return_block)
print('Done!')
